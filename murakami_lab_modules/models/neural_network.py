...
The built-in networks are intentionally modest wrappers around
``torch.nn.Module``. Users can pass any compatible PyTorch module to
``ModelHandler``; these classes mainly provide convenient defaults and
serializable configuration metadata.
"""
...
class BaseNeuralNetwork(torch.nn.Module):
    """Base class for the built-in fully connected networks.

    Parameters define a standard multilayer perceptron. ``activation`` may be a
    ``torch.nn.Module`` instance, a callable, or ``None``. Module activations are
    deep-copied for each hidden layer so stateful activations are not shared.
    """
    ...
    def get_neural_network_model(
        ...
        """Create a sequential feed-forward network from the given dimensions."""
...
class FeedForwardNeuralNetwork(BaseNeuralNetwork):
    """Fully connected network with signature ``forward(x)``."""
...
class ODEFeedForwardNeuralNetwork(BaseNeuralNetwork):
    """Fully connected network with ODE-compatible signature ``forward(t, x)``.

    When ``use_time_as_input`` is true, the scalar or batched time value is
    concatenated to ``x`` before the network evaluation. Otherwise ``t`` is
