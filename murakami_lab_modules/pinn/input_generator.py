"""Input generation utilities for collocation points and synthetic samples."""
...
class InputGenerator:
    """Generate inputs for PINN regularization or synthetic evaluation.

    Parameters
    ----------
    n_samples:
        Number of samples returned by each call.
    input_range:
        Sequence of ``(min, max)`` pairs, one per input dimension.
    sampling:
        ``"random"`` for uniform random samples, ``"sobol"`` for Sobol
        low-discrepancy samples, or ``"fixed"`` for user-provided inputs.
    scale:
        ``"linear"`` or ``"log"`` per dimension.
    filter_func:
        Optional rejection filter. It must return a boolean mask for candidate
        samples.
    resample:
        When false, generated samples are cached and reused.
    requires_grad:
        Controls whether returned inputs are suitable for autograd-based
        residuals.
    """
    ...
    def config_dict(self) -> dict[str, object]:
        """Return serializable metadata for this generator."""
    ...
    def __call__(self) -> torch.Tensor:
        """Return generated inputs with the configured ``requires_grad`` flag."""
