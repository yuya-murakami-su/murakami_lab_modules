"""PINN-style regularization terms and automatic weight calibration."""
...
class RegularizationWeightPolicy:
    """Base class for policies that initialize or update regularization weights."""
...
class StaticRegularizationWeights(RegularizationWeightPolicy):
    """Use fixed regularization weights."""
...
class TargetTotalRegularizationWeight(RegularizationWeightPolicy):
    """Choose weights so the initial weighted regularization sum matches a target."""
...
class MatchDataLossRegularizationWeight(TargetTotalRegularizationWeight):
    """Choose weights relative to the current data loss.

    The initial target total is ``alpha * data_loss``. This is useful when
    regularization magnitudes are not known before training.
    """
...
class Regularization:
    """Base class for PINN or physics-inspired regularization.

    Subclasses normally override ``regularization(self, data_handler, nn)`` and
    return a list or tuple of residual tensors. Each residual is transformed by
    ``penalty_fn``, averaged independently, weighted, and then added to the data
    loss by ``ModelHandler``.

    The derivative helpers ``grad``, ``partial``, ``partial2``, ``jacobian``,
    ``hessian_diag``, and ``laplacian`` are available as static methods for
    convenient subclass implementations.
    """
    ...
    def regularization(self, data_handler: DataHandler, nn: BaseNeuralNetwork):
        """Return residual tensors for this regularization.

        Subclasses must implement this method unless ``method_name`` points to a
        different method.
        """
    ...
    def compute_raw_term_means(
        ...
        """Compute unweighted mean penalty values for each regularization term."""
    ...
    def combine_regularization_terms(
        ...
        """Apply weights to raw term means and return terms plus total loss."""
    ...
    def calibrate_weights(
        ...
        """Evaluate residuals and initialize weights from the configured policy."""
    ...
    def save_weight_report(self, path: str | Path, **to_csv_kwargs) -> None:
        """Save the current regularization weight report as CSV."""
    ...
    def get_regularization_value(
        ...
        """Return weighted term values and total regularization loss."""
