"""Small loss helpers not covered directly by standard PyTorch modules."""
...
def relative_mse_loss(
    ...
    """Return squared relative error loss.

    The default reduction is ``"mean"`` so the result can be passed directly to
    ``backward()`` as a scalar loss.
    """
...
def component_weighted_mse_loss(
    ...
    """Return MSE with one weight per output component."""
