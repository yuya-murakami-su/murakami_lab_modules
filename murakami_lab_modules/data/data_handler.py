"""High-level data preparation for training workflows.

``DataHandler`` loads inputs, outputs, and optional labels; creates train,
validation, and test splits; fits normalizers on the training split; and exposes
iterable data loaders for each split. It is designed for small to medium
scientific datasets where keeping tensors on the target device is practical.
"""
...
class DataHandler:
    """Load, split, normalize, and batch model data.

    Parameters
    ----------
    input_data_path:
        Path to a CSV, NumPy, or PyTorch data file containing inputs. When
        ``output_data_path`` or ``label_data_path`` is omitted, the same file is
        used for those values.
    input_columns, output_columns, label_columns:
        Column names or integer indices for tabular data. String column names
        are supported for CSV files.
    input_key, output_key, label_key:
        Keys used when reading dictionary-like ``.npz``, ``.npy``, ``.pt``, or
        ``.pth`` files.
    split_type:
        ``"random_split"`` uses ``split_ratio``. ``"index_split"`` uses
        ``train_indices``, ``valid_indices``, and ``test_indices`` passed via
        keyword arguments.
    dataloader_type:
        ``"native"`` uses the lightweight in-memory loader. ``"torch"`` uses a
        ``torch.utils.data.DataLoader`` adapter.
    ...
    def from_tensors(
        ...
        """Build a ``DataHandler`` from in-memory arrays, tensors, or lists.

        This constructor is convenient for tests, generated data, and workflows
        where data has already been loaded by project-specific code.
        """
    ...
    def config_dict(self) -> dict[str, object]:
        """Return JSON-serializable metadata describing this data pipeline."""
    ...
    def summary_dict(self, max_label_counts: int = 20) -> dict[str, object]:
        """Return a lightweight human-readable summary of loaded data.

        The summary records paths, columns, shapes, split sizes, normalizers, and
        label counts when the number of unique labels is small.
        """
    ...
    def save_summary(self, path: str | Path, max_label_counts: int = 20) -> None:
        """Save the data summary as JSON or flattened CSV based on file suffix."""
    ...
    def __call__(self, dataset_name: str):
        """Yield batches for ``dataset_name``.

        Valid names are usually ``"train"``, ``"valid"``, ``"test"``,
        ``"all"``, and optionally ``"train_valid"``.
        """
