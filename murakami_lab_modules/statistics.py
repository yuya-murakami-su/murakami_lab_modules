...
def conduct_pca(
    ...
    if save_scores:
        ...
        scores.to_csv(save_path / 'pca_scores.csv')

    if save_loadings:
        loadings = pd.DataFrame(pca.components_.T,
                                index=df.columns,
                                columns=[f'PC{i+1}' for i in range(pca.n_components_)])
        loadings.to_csv(save_path / 'pca_loadings.csv')
    ...
    if save_pc_scatter:
        ...
        plt.tight_layout()
        plt.savefig(save_path / 'pc1_pc2_scatter.png', dpi=300)
        plt.close()
    ...
    if save_biplot and pca.n_components_ >= 2:
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(save_path / 'biplot.png', dpi=300)
        plt.close()
