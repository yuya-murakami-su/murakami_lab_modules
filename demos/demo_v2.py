...
def main():
    ...
    pd.DataFrame(np.vstack(x_data), columns=('x1', 'x2')).to_csv('demo_data\\demo_v2_x.csv')
    pd.DataFrame(np.hstack(y_data)[:, None], columns=('y',)).to_csv('demo_data\\demo_v2_y.csv')

    # Define data handler for control datasets / データセット制御クラスの定義
    data_handler = DataHandler(
        input_data_path='demo_data\\demo_v2_x.csv',
        input_idx=['x1', 'x2'],
        output_data_path='demo_data\\demo_v2_y.csv',
        output_idx=['y'],
        batch_size=8,
        device_name='cpu',
    ...
    plotter.add_details(title='Demo v2', x_label=r'$x$', y_label=r'$y$', x_lim=(0, 10), legend_outside=True)
    plotter.save_fig('demo_v2')
    plotter.display()
...
class MyRegularization(Regularization):
    def regularization(self, data_handler: DataHandler, nn: AbstractNeuralNetwork):
        ...
        d2y_dx1 = self.grad(dy_dx1, generated_input, x_idx=1)
        return [
            dy_dx1.clamp_max(max=0),  # dy_dx1 >= 0
            d2y_dx1.clamp_min(min=0)  # d2y_dx1 <= 0
        ]
